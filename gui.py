...
model = ImageModel('./my.db')
uploader = ImageUploader()
uploader.load_credential()
#uploader.set_imgur_api('6b75af0f94d844f','0f45fb3d25a709ee76aabd9d4d72957b8c839985')
...
class GuiSample(object):
    ...
    def __init__(self):
        ...
        self.add_new_window_Upload.grid(row=1, column=1, sticky=tk.W)
        self.add_new_window_Search.grid(row=2, column=1, sticky=tk.W)
        self.add_new_window_all_photo.grid(row=3, column=1, sticky=tk.W)
        self.quit_button.grid(row=4, column=1, sticky=tk.W)
        self.check_button.grid(row=5, column=0, sticky=tk.W)
    ...
    def build_button(self):
        ...
        tk.Label(self.root, text='请在此处输入账号').grid(row=1, column=0)
        self.account_text=tk.StringVar()

        self.account_entry = tk.Entry(self.root,text=self.account_text)
        self.account_entry.grid(row=2, column=0)
        tk.Label(self.root, text='请在此处输入密码').grid(row=3, column=0)
        self.password_text = tk.StringVar()
        self.password_entry = tk.Entry(self.root,text=self.password_text)
        self.password_entry.grid(row=4, column=0)

        if uploader.load_credential() ==0:
            self.account_text.set(uploader.imgur_client_id)
            self.password_text.set(uploader.imgur_client_secret)

        self.check_button = tk.Button(self.root, text='Check', command=self.check_ac, bg='DeepPink')
    ...
    def check_ac(self):
        uploader.set_imgur_api(self.account_text.get(),self.password_text.get())
        uploader.save_credential()
    ...
    def new_window_Upload(self):
        ...
        self.alias_text = tk.Entry(self.window)
        self.alias_text.grid(row=9, column=0)
